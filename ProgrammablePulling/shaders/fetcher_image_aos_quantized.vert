...
layout(r8, binding = 1) restrict readonly uniform imageBuffer normalBuffer;
...
void main(void) {
	...
	vec3 inVertexNormal;
	inVertexNormal.x   = imageLoad(normalBuffer, gl_VertexID * 3 + 0).x * 2.0 - 1.0;
	inVertexNormal.y   = imageLoad(normalBuffer, gl_VertexID * 3 + 1).x * 2.0 - 1.0;
	inVertexNormal.z   = imageLoad(normalBuffer, gl_VertexID * 3 + 2).x * 2.0 - 1.0;
