...
layout(r8, binding = 4) restrict readonly uniform imageBuffer normalXBuffer;
layout(r8, binding = 5) restrict readonly uniform imageBuffer normalYBuffer;
layout(r8, binding = 6) restrict readonly uniform imageBuffer normalZBuffer;
...
void main(void) {
	...
	vec3 inVertexNormal;
	inVertexNormal.x   = imageLoad(normalXBuffer, inIndex).x * 2.0 - 1.0; 
	inVertexNormal.y   = imageLoad(normalYBuffer, inIndex).x * 2.0 - 1.0; 
	inVertexNormal.z   = imageLoad(normalZBuffer, inIndex).x * 2.0 - 1.0; 
